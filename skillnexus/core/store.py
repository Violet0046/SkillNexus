"""
Storage location: <project_root>/.skillnexus/skillnexus.db
Tables:
...
class SkillStore:
    """SQLite persistence engine for skill quality tracking and evolution."""

    def __init__(self, db_path: Optional[Path] = None) -> None:
        if db_path is None:
            db_dir = PROJECT_ROOT / ".skillnexus"
            db_dir.mkdir(parents=True, exist_ok=True)
            db_path = db_dir / "skillnexus.db"
    ...
    async def sync_from_registry(self, discovered_skills: List[Any]) -> int:
        return await asyncio.to_thread(self._sync_from_registry_sync, discovered_skills)

    @_db_retry()
    def _sync_from_registry_sync(self, discovered_skills: List[Any]) -> int:
        self._ensure_open()
        created = 0
        refreshed = 0
        with self._mu:
    ...
    def _evolve_skill_sync(
        self, new_record: SkillRecord, parent_skill_ids: List[str],
    ) -> None:
        self._ensure_open()
        with self._mu:
            self._conn.execute("BEGIN")
    ...
    def load_analyses_for_task(self, task_id: str) -> Optional[ExecutionAnalysis]:
        with self._reader() as conn:
            row = conn.execute(
                "SELECT * FROM execution_analyses WHERE task_id=?", (task_id,),
            ).fetchone()
    ...
    def _subtree(
        self, conn: sqlite3.Connection, sid: str, depth: int, visited: set,
    ) -> Dict[str, Any]:
        visited.add(sid)
        row = conn.execute(
