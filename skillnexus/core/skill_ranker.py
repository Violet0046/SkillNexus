...
Embedding strategy:
  - Text = ``name + description + SKILL.md body``
  - Model: configurable via settings (default: text-embedding-3-small)
...
def _resolve_embedding_api() -> tuple[Optional[str], str]:
    """Resolve embedding API key and base URL from environment variables.

    Checks (in order):
      1. EMBEDDING_API_KEY / EMBEDDING_API_BASE
      2. OPENAI_API_KEY / OPENAI_API_BASE
      3. OPENROUTER_API_KEY (base defaults to https://openrouter.ai/api/v1)

    Returns:
        (api_key, base_url) — api_key may be None if not configured.
    """
    # Dedicated embedding env vars
...
class SkillCandidate:
    ...
    # Pass-through metadata (for search results)
...
class SkillRanker:
        ...
        *,
        ...
        if cache_dir is None:
            try:
                ...
                cache_dir = PROJECT_ROOT / ".skillnexus" / "skill_embedding_cache"
            except Exception:
                cache_dir = Path(".skillnexus") / "skill_embedding_cache"
    ...
    def _build_embedding_text(candidate: SkillCandidate) -> str:
        """Build text for embedding: name + description + body."""
    ...
    def _embedding_rank(
        ...
        api_key, _ = _resolve_embedding_api()
        if not api_key:
            return []
    ...
    def _generate_embedding(
        ...
        Uses env vars for API key and base URL resolution.
        """
        resolved_key, base_url = _resolve_embedding_api()
        if not api_key:
